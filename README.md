...
如果这个脚本对你有帮助，欢迎在 GitHub 右上角点一个 Star。
...
**新增功能**
- `-InstallDir` 参数支持自定义安装目录
- 安装日志文件（`logs\setup_yyyyMMdd_HHmmss.log`），记录完整安装过程，API Key 自动打码
- 安装完成后显示详细总结（安装目录、版本、模型配置等）
- 卸载日志文件（`%TEMP%\ClaudeCodeCLI_uninstall_logs\`）
- 下载失败自动重试（最多 2 次，间隔递增）

**问题修复**
- 修复控制台中文输出乱码/重复字符
- 修复 `[警告] 无法设置执行策略` 误报（执行策略已正确时不再提示）
- 修复 `[警告] npm 进程退出码异常` 误报（npm 确认成功时静默记录日志）
- 卸载时智能检测 PATH 中的真实安装目录，忘记传 `-InstallDir` 也能给出提示
- 卸载前检测运行中的 Claude Code 进程并提醒
- 安装后清理 npm 代理配置和临时文件
